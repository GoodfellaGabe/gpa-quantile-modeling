...
- $X_i$ = the (1 x $p$) vector of predictors for observation i. An (n x $p$) design matrix represents the stack of all observations.
- $\beta_\tau$ = the ($p$ x 1) coefficient vector (a different $\beta_\tau$ is estimated differently at each $\tau$).
...
Nonnegative residuals are weighted by $\tau$; negative residuals by ($\tau$-1). Minimizing the sum of these losses yields $\hat{\beta}_\tau$.The asymmetric weights adjust the model fit so that about a fraction $\tau$ of residuals are $\le$ 0 at the solution, targeting the conditional $\tau^{th}$ quantile rather than the mean. <br>
...
Since coefficients in an OLS model are solved with $\hat{\beta}$ = $(X^TX)^{-1}X^TY$, then:
$$
\hat{\beta} = (X^TX)^{-1}X^T \times (X\beta+\epsilon) = \beta + (X^TX)^{-1}X^T\epsilon
$$

Classical OLS inference assumes its expected errors to be zero ($\epsilon | X$)=0 and constant variance $Var(\epsilon |  X) = \sigma^2I$ where $I$ is the identity matrix $(X^TX)^{-1}$. Under the constant variance assumption, we have: 
$$
Var(\hat{\beta} | X) = \sigma^2I = \sigma^2(X^TX)^{-1}
$$
Since in our case the true student population variance ($\sigma^2$) is unknown, we have to estimate it from our the sample dataset provided by the district. The formula for estimating the sample variance is ($\hat{\sigma^2}$):
$$
\hat{\sigma}^2 = \frac{\text{RSS}}{n - p} = \frac{\sum_{i=1}^n \hat{r}_i^2}{n - p}
$$
where n=sample size (649 students) and $p$=size of vector $\hat{\beta}$ = 1 + number of predictors in the OLS model.
...
If we observe male students, compared to the bottom 10%, we see they are slightly more associated with lower GPAs than the middle. Holding all other variables fixed, the $10^{th}$-quantile GPA for male students is about 0.166 units lower than for comparable females. However, at the $50^{th}$-quantile, the median GPA for males is about 0.110 units lower than for comparable females. 
<br>
